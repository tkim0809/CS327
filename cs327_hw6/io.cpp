...
void fly(pair_t dest)
{
  ...
  mvprintw(0, 0, "Enter x coordinate and hit enter(use x coordinate within -200~200): ");
  refresh();
  echo();
  curs_set(1);
  char buf[10];
  wgetnstr(stdscr, buf, 9);
  x_str = buf;
  x = std::stoi(x_str);
  mvprintw(0, 0, "Enter y coordinate and hit enter(use y coordinate within -200~200): ");
  refresh();
  wgetnstr(stdscr, buf, 9);
  y_str = buf;
  y = std::stoi(y_str);
