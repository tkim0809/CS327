...
void pokemon_popup() {
  ...
  double manhattanDistance = abs(world.cur_idx[dim_x] - 200) + abs(world.cur_idx[dim_y] - 200);
  vector<int> ivs;
  ...
  int match = 0;
    ...
    for(match = 0; match < 10194; match++)
    {
      if(pokemon_stats[match].pokemon_id == random)
    ...
    else if(manhattanDistance > 200) {
      
        level = (rand() % (int)((100 - ((manhattanDistance - 200) / 2) + 1)) + ((manhattanDistance - 200) / 2));
    }
    else {
        level = (rand() % (int)(manhattanDistance / 2)) + 1;
    }

    int basehp = pokemon_stats[match].base_stat;
    int baseattack = pokemon_stats[match].base_stat;
    int basedefense = pokemon_stats[match++].base_stat;
    int basespecialattack = pokemon_stats[match++].base_stat;
    int basesepcialdefense = pokemon_stats[match++].base_stat;
    int basespeed = pokemon_stats[match++].base_stat;

    int hp = (((basehp + ivs[0]) * 2) * level) / 100 + level + 10;
    int attack = (((baseattack + ivs[1]) * 2 * level) / 100 + 5);
    int defense = (((basedefense + ivs[2]) * 2 * level) / 100 + 5);
    int specialAttack = (((basespecialattack + ivs[3]) * 2 * level) / 100 + 5);
    int specialDefense = (((basesepcialdefense + ivs[4]) * 2 * level) / 100 + 5);
    int speed = (((basespeed + ivs[5]) * 2 * level) / 100 + 5);

     int moves_start = -1;
     int moves_end = -1;
    
    for(int i = 0; i < 528238; i++) {
        if(pokemon_moves[i].pokemon_id == pokemon[random].species_id) {
...
int  move_two_id = 0;
 ...
 if(rand() % 8192 == 0)
 {
  ...
  mvprintw(0,0, "%s wants to fight you!", pokemon[random].identifier);
  ...
  mvprintw(4,0, "id: %d", pokemon[random].id);
  mvprintw(5,0, "SHINY: %s", shiny ? "SHINY" : "NOT SHINY");
  mvprintw(6,0, "gender: %s", gender ? "female" : "male");
  mvprintw(7,0, "HP: %d", hp);
  mvprintw(8,0, "Attack: %d", attack);
  mvprintw(9,0, "Defense: %d", defense);
  mvprintw(10,0, "Special Attack: %d", specialAttack);
  mvprintw(11,0, "Special Defense: %d", specialDefense);
  mvprintw(12,0, "Speed: %d", speed);
