...
static void dijkstraDistanceMap(map_t *m)
{
  ...
  while ((p = heap_remove_min(&h))) {
    p->hn = NULL;
      ...
      //Top left
      if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) && (path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost > (p->cost + m->costMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1]))) {

        path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost = ((p->cost + m->costMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1]));
    ...
    //Left
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) && (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost > (p->cost + m->costMap[p->pos[dim_y]][p->pos[dim_x] - 1]))) {

        path[p->pos[dim_y]][p->pos[dim_x] - 1].cost = ((p->cost + m->costMap[p->pos[dim_y]][p->pos[dim_x] - 1]));
    ...
    // Top 
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) && (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost > (p->cost + m->costMap[p->pos[dim_y] - 1][p->pos[dim_x]]))) {

        path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost = ((p->cost + m->costMap[p->pos[dim_y] - 1][p->pos[dim_x]]));
...
void calculateWeights(map_t *m, char T) {
    ...
    case 'H':
      ...
      for (int i = 0; i < MAP_Y; i++) {
        ...
        for (int j = 0; j < MAP_X; j++) {
            ...
            if (m -> map[i][j] == ter_path) {

                m->costMap[i][j] = 10;

            } else if (m -> map[i][j] == ter_mart) {

                m->costMap[i][j] = 50;

            } else if (m -> map[i][j] == ter_center) {

                m->costMap[i][j] = 50;

            } else if (m -> map[i][j] == ter_clearing) {

                m->costMap[i][j] = 10;

            } else if (m -> map[i][j] == ter_grass) {

                m->costMap[i][j] = 15;

            } else if (m -> map[i][j] == ter_mountain) {

                m->costMap[i][j] = 15;

            } else if (m -> map[i][j] == ter_forest) {

                m->costMap[i][j] = 15;

            } else if (m -> map[i][j] == ter_water || m -> map[i][j] == ter_gate || m-> map[i][j] == ter_boulder || m->map[i][j] == ter_tree) {
      ...
      for (int i = 0; i < MAP_Y; i++) {
        ...
        for (int j = 0; j < MAP_X; j++) {
            ...
            } else if (m -> map[i][j] == ter_water || m -> map[i][j] == ter_gate || m-> map[i][j] == ter_boulder || m->map[i][j] == ter_tree || m->map[i][j] == ter_mountain || m->map[i][j] == ter_forest) {
...
int main(int argc, char *argv[])
{
  ...
  calculateWeights(world.cur_map, 'H');
  dijkstraDistanceMap(world.cur_map);
  calculateWeights(world.cur_map, 'R');
  dijkstraDistanceMap(world.cur_map);
